...
describe('Relayer API Wrapper', () => {
    ...
    it('should call createSession with correct parameters', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/api/session/create', {
            method: 'POST',
    ...
    it('should call buyShares with correct parameters and hit /api/trade/buy', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/api/trade/buy', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
