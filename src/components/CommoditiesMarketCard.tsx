...
const CommoditiesMarketCard = memo(({ market }: CommoditiesMarketCardProps) => {
    ...
    // Smart image resolver: use Polymarket image if valid, otherwise pick a keyword-based Unsplash photo
    const getCommodityImage = (): string => {
        const src = market.image;
        const isUsable = src && !src.includes('polymarket-logo') && !src.startsWith('/images/markets/');
